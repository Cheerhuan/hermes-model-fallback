---
name: hermes-model-fallback
description: >
  Handles AI model failures (HTTP 429, HTTP 503, Timeout) by automatically switching
  to a backup model without freezing or crashing. Use this skill whenever you need
  resilient multi-model API calls, rate-limit protection, or automatic failover between
  different AI providers (Gemini, OpenAI, Claude, Mistral, etc.). Trigger when the user
  mentions "fallback", "model switch", "rate limit", "429", "503", "timeout retry",
  "multi-model", or wants their agent to be fault-tolerant against API errors.
---
